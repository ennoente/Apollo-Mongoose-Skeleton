...
import { wrap, ensure, unthunk, inspect, checkFilterValidity } from '../util'
...
export function populateASTNode(queryASTNode, parentTypeNode, sqlASTNode, namespace, depth, options, context) {
    ...
    */
    ...
    //console.log("field.sqlColumn", field.sqlColumn);
    //console.log("field.bla", field.bla);

    if (field.permissions) {
        sqlASTNode.permissions = field.permissions;
    }

    //if (field.fieldMappings) {
    //    console.log(`Field ${field.name} has fieldMappings object`);
    //    sqlASTNode.fieldMappings = field.fieldMappings;
    //}


    // is this a table in SQL?
    if (!field.jmIgnoreTable &&
        ...
        //sqlASTNode.parent = parentTypeNode;
...
function handleTable(sqlASTNode, queryASTNode, field, gqlType, namespace, grabMany, depth, options, context) {
    ...
    if (field.sqlJoin) {
        if (filter) {
            ...
            //sqlASTNode.filteredOneToManyJoinParent = sqlASTNode.parent;

            //sqlASTNode.filteredJoin = field.sqlJoin;
            //sqlASTNode.filteredJoinDeps = unthunk(field.filteredJoinDeps, sqlASTNode.parent.as, sqlASTNode.as);
        }
        ...
        if (filter) {
            ...
            sqlASTNode.filteredManyToMany = true;
        }
        ...
        if (field.junction.sqlJoins) {
            ...
            children.push({
                ...
                ...keyToASTChild(ensure(field.junction, 'uniqueKey'), namespace, sqlASTNode),
            ...
            junction.sqlBatch = {
                ...
                thisKey: {
                    ...
                    //...columnToASTChild(ensure(field.junction.sqlBatch, 'thisKey'), namespace, sqlASTNode.parent),
                    fromOtherTable: junction.as
                },
                //parentKey: columnToASTChild(ensure(field.junction.sqlBatch, 'parentKey'), namespace)
                parentKey: columnToASTChild(ensure(field.junction.sqlBatch, 'parentKey'), namespace, sqlASTNode.parent)
            }
        } else {
        ...
        sqlASTNode.sqlBatch = {
            ...
            thisKey: columnToASTChild(ensure(field.sqlBatch, 'thisKey'), namespace, sqlASTNode.parent),

            //parentKey: columnToASTChild(ensure(field.sqlBatch, 'parentKey'), namespace)
            parentKey: columnToASTChild(ensure(field.sqlBatch, 'parentKey'), namespace, sqlASTNode.parent),
        }
    } else if (filter) {
        // Toplevel-field
        sqlASTNode.filteredToplevelField = true;

        // Has to be filtered on WHERE clause (right?!)
        if (filter) {
            sqlASTNode.filteredWhereMap = createJoinMap(
                sqlASTNode,
                queryASTNode,
    ...
    children.push(keyToASTChild(ensure(config, 'uniqueKey'), namespace, sqlASTNode));
    ...
    if (config.alwaysFetch) {
        for (let column of wrap(config.alwaysFetch)) {
            ...
            children.push(columnToASTChild(unthunk(column, sqlASTNode.as, sqlASTNode.args || {}, context, sqlASTNode), namespace, sqlASTNode.parent))
        }
    }
    ...
    if (config.typeHint && ['GraphQLUnionType', 'GraphQLInterfaceType'].includes(gqlType.constructor.name)) {
        ...
        children.push(columnToASTChild(config.typeHint, namespace, sqlASTNode.parent))
    }
...
function _createJoinMap(
    ...
    // Could also be just a single `compare` object. To iterate, wrap it in an array
    ...
    for (let index in filterArr) {
        ...
        if (current.OR) {
            ...
            /** Here we have the inidivual compare objects
             * 
             * We need to loop thourh the key's parts and check if we
             * need to add anything to the selections and join tables.
             * 
            */ 
            ...
            let field;
            let type = gqlType;
            let currentJAstNode = { ...jAst };

            parts.forEach(part => {
                [field, type] = getCorrespondingFieldAndType(part, type, jAst.fieldName);
                const existingNode = currentJAstNode.children.find(child => child.fieldName === part);

                // If the currentType represents a sql table add it to the JAst
                if (idx(type, _ => _._typeConfig.sqlTable)) {
                    if (!existingNode) {
                        const newNode = new SQLASTNode(currentJAstNode, {
                            type: 'table',
                            name: type._typeConfig.sqlTable,
                            as: namespace.generate('table', type._typeConfig.sqlTable),
                            fieldName: part,
                            children: []
                        })
                        ...
                        if (field.sqlJoin) {
                            ...
                            const junctionTable = unthunk(ensure(field.junction, 'sqlTable'), node.args || {}, context);
            ...
            /*
            console.log("type after iterating through parts:", currentType);
            console.log("Jast:", jAst);
            */
...
function getCorrespondingFieldAndType(part, gqlType, rootFieldName) {
    const correspondingField = idx(gqlType, _ => _._fields[part]);

    if (!correspondingField)
        throw new Error(`Wrong filter part '${part}', the type '${gqlType.name}' does not contain this field.` +
                        ` Check your filter at the '${rootFieldName}' field`);

    const correspondingFieldType = correspondingField.type;

    if (!correspondingFieldType)
        throw new Error(`Wrong filter at part '${part}' for type '${gqlType.name}'`);

    // If the type is NonNull the type will be GraphQLNonNull and the actual type will the
    // the `ofType` property
    if (correspondingFieldType.ofType)
        return [correspondingField, correspondingFieldType.ofType];
    else
        return [correspondingField, correspondingFieldType];
}
...
function columnToASTChild(columnName, namespace, parent) {
    //console.log("namespace", namespace);
    //console.log("parent", parent);
    //console.trace(`columnName=${columnName}`);
...
function __columnToASTChild(columnName, namespace) {
    return {
        type: 'column',
        name: columnName,
        fieldName: columnName,
...
function keyToASTChild(key, namespace, parent) {
    if (typeof key === 'string') {
        //return columnToASTChild(key, namespace)
        return columnToASTChild(key, namespace, parent);
    }
