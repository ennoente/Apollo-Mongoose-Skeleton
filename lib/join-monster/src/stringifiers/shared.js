...
export function getFilteredTableParentNode(node) {
  let { parent } = node;

  if (!parent)
    return null;

  if (parent.filteredOneToManyJoin) {
    return parent;
  }

  return getFilteredTableParentNode(parent);
}

export function nodeIsChildOfFilteredTable(node) {
  let { parent } = node;

  if (!parent) {
    console.log(`Node ${node.as} is NOT child of filtered table`);
    return false;
  }
  ...
  return nodeIsChildOfFilteredTable(parent);
}
...
export function tableHasNoFilteredOneToManyJoin(node) {
  console.log("node in tableHas...", node);
  return node.filteredOneToManyJoin ? false : true;
}
