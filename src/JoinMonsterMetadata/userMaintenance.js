import { addJmType, extendJmQuery, extendJmMutation } from 'join-monster-modularizer';

addJmType('UserMaintenance', {
...
extendJmQuery('allUserMaintenances', {
    limit: ({ first }) => first,
    orderBy: ({ orderBy }) => orderBy || 'id',
    authorization: ({ userId }, ctx) => ({
        userId: {
            credentialsMatch: () => [ userId === ctx.jwt.userId, 'Not authorized' ]
        }
    }),
    where: (mTable, args, ctx) => {
        const { userId, beyond } = args;
        return `${mTable}.sys_user_id = ${userId} ${beyond ? ` AND ${mTable}.id > ${beyond}` : ''}`
    }
})

extendJmMutation(['createUserMaintenance', 'updateUserMaintenance', 'deleteUserMaintenance'], {
