...
(0, _joinMonsterModularizer.addJmType)('User', {
  sqlTable: "sys_user",
  uniqueKey: "id",
  fields: {
    email: {
    ...
    machines: {
      ...
      sqlJoin: (userTable, userMachineTable) => `${userTable}.id = ${userMachineTable}.sys_user_id`
    },
    room: {
    ...
    rooms: {
      ...
      sqlJoin: (userTable, userRoomTable) => `${userTable}.id = ${userRoomTable}.sys_user_id`
    },
    maintenance: {
    ...
    maintenances: {
      ...
      sqlJoin: (userTable, userMTable) => `${userTable}.id = ${userMTable}.sys_user_id`
    },
    dueMaintenance: {
      //where: (table, { id }) => `${table}.id = ${id}`
      sqlJoin: (u, m, {
        id
      }) => `${u}.id = ${m}.sys_user_id AND ${m}.id = ${id} `
    },
    dueMaintenances: {
      sqlJoin: (userTable, dmTable) => `${userTable}.id = ${dmTable}.sys_user_id`
    },
    finishedMaintenance: {
      sqlJoin: (userTable, fmTable, {
        id
      }) => `${userTable}.id = ${fmTable}.sys_user_id AND ${fmTable}.id = ${id}`
    },
...
(0, _joinMonsterModularizer.extendJmQuery)('me', {
